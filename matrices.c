...
int main(int argc,char*argv[]){
 ...
 //controla los argumentos al programa
	...
	if (print == 1) {
		...
		for(i=0;i<n;i++) {
			for(j=0;j<n;j++) 
				printf("%f\t",getValor(b,i,j,n,ORDENXFILAS));
			printf("\n");
		}
		printf("\n");
	...
	for(i=0;i<n;i++){
		for(j=0;j<n;j++){
			...
			for(k=0;k<n;k++){
				setValor(c,i,j,n,ORDENXFILAS, getValor(c,i,j,n,ORDENXFILAS) + getValor(a,i,k,n,ORDENXFILAS)*getValor(b,k,j,n,ORDENXFILAS));
			}
		}
	}   
